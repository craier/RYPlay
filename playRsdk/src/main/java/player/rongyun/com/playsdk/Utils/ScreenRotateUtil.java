...
//                        SettingManager.getInstance().setLandPortMode(SettingManager.LAND_MODE);
//                        isLandscape = true;
//                    }
                } else if (orientation > 135 && orientation < 225) {
//                    mActivity.setRequestedOrientation(ActivityInfo
// .SCREEN_ORIENTATION_REVERSE_PORTRAIT);
//                    SettingManager.getInstance().setLandPortMode(SettingManager.PORT_MODE);
//                    isLandscape = false;
     ...
     */
    private void updateState(boolean isLandscape, boolean isClickFullScreen, boolean
            isOpenSensor, boolean isChangeOrientation) {
    ...
    private boolean screenIsLandscape(int orientation) {
        return ((orientation > 45 && orientation <= 135) || (orientation > 225 && orientation <=
                315));
    }
    ...
    private boolean screenIsPortrait(int orientation) {
        return (((orientation > 315 && orientation <= 360) || (orientation >= 0 && orientation <=
                45))
    ...
    public void stop() {
        ...
        isClickFullScreen = false;        // 记录全屏按钮的状态，默认false
        isOpenSensor = true;      // 是否打开传输，默认打开
        isLandscape = false;      // 默认是竖屏
        isChangeOrientation = true;  // 记录点击全屏后屏幕朝向是否改变，默认会自动切换
        isEffetSysSetting = false;
    }
    ...
    public void toggleRotate() {
        ...
        if (isEffetSysSetting) {
            try {
                int isRotate = Settings.System.getInt(mActivity.getContentResolver(), Settings
