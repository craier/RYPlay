package com.rongyun.lib_screen.manager;


import com.rongyun.lib_screen.VError;
import com.rongyun.lib_screen.callback.AVTransportCallback;
import com.rongyun.lib_screen.callback.ControlCallback;
import com.rongyun.lib_screen.callback.RenderingControlCallback;
...
import com.rongyun.lib_screen.utils.ClingUtil;
...
public class ControlManager {
    ...
    private RenderControlEvent renderControlEvent;
     ...
     */
    public void newPlayCast(final LocalItem item, final ControlCallback callback, final String speed) {
     ...
     */
    public void newPlayCast(final RemoteItem item, final ControlCallback callback, final String
            speed) {
        stopCast(new ControlCallback() {
            @Override
            public void onSuccess() {
     ...
     * 播放投屏
     */
    public void playCast(final ControlCallback callback, String speed) {
        if (checkAVTService()) {
        ...
        controlPoint.execute(new Play(instanceId, avtService, speed) {
            @Override
     ...
     * @param item 需要投屏的资源
     */
    public void setAVTransportURI(LocalItem item, final ControlCallback callback) {
        if (checkAVTService()) {
    ...
    public void setAVTransportURI(RemoteItem item, final ControlCallback callback) {
        ...
        ControlPoint controlPoint = ClingManager.getInstance().getControlPoint();
        controlPoint.execute(new SetAVTransportURI(instanceId, avtService, item.getUrl(),
                metadata) {
            @Override
            public void success(ActionInvocation invocation) {
    ...
    public void initScreenCastCallback() {
        ...
        avtCallback = new AVTransportCallback(avtService) {
            ...
            protected void received(AVTransportInfo info) {
                ...
                if (renderControlEvent != null) {
                    renderControlEvent.onRenderControlEvent(event);
                }
            }
        };
        ...
        rcCallback = new RenderingControlCallback(rcService) {
            ...
            protected void received(RenderingControlInfo info) {
                LogUtils.d("RenderingControlCallback received: mute:%b, volume:%d", info.isMute()
                        + "   volume  " + info
                        .getVolume());
                ControlEvent event = new ControlEvent();
    ...
    public void setRenderControlEvent(RenderControlEvent renderControlEvent) {
        this.renderControlEvent = renderControlEvent;
    }
    ...
    public interface RenderControlEvent {
        void onRenderControlEvent(ControlEvent event);
    }
}
